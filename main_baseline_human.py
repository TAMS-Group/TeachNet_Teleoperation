...
def train(model, loader, epoch):
    ...
    for batch_idx, (shadow, human, target) in enumerate(loader):
        ...
        loss_human = loss_human_reg + loss_human_cons
        loss_human.backward()
        optimizer.step()
        ...
        if batch_idx % args.log_interval == 0:
            ...
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}\tLoss_reg_human: {:.6f}\t'
                    'Loss_cons_human: {:.6f}\t{}'.format(
                    epoch, batch_idx * args.batch_size, len(loader.dataset),
                    100. * batch_idx * args.batch_size / len(loader.dataset),
                    loss.item(), loss_human_reg.item(), loss_human_cons.item(), args.tag))
...
def test(model, loader):
    ...
    test_loss = test_loss_human_reg + test_loss_human_cons
    ...
    return acc_human, test_error_human, test_loss, test_loss_human_reg, test_loss_human_cons
...
def main():
    if args.mode == 'train':
        for epoch in range(is_resume*args.load_epoch, args.epoch):
            ...
            print('Train done, acc_human={}, train_error_human={}'.format(acc_train_human, train_error_human))
            acc_test_human, test_error_human, loss, loss_human_reg, loss_human_cons = test(model, test_loader)
            print('Test done, acc_human={}, error_human={}, loss={}, loss_human_reg={}, '\
                  'loss_human_cons={}'.format(acc_test_human, test_error_human, loss, loss_human_reg,
                                              loss_human_cons))
